...
class Gate(nn.Module):
    ...
    def forward(self, x, epochs, top_k):
        ...
        if epochs < self.threshod_epochs:
            gate_top_k_idx = torch.randint(1, self.num_of_experts + 1, (x.size(0), 1))
        else:
            gap = self.gate(gap)
...
class Generic_UNet_MOE(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 seg_output_use_bias=False, num_of_experts=3):
        """
        ...
        """
        super(Generic_UNet_MOE, self).__init__()
        ...
        self.tu = nn.ModuleList([nn.ModuleList(self.tu) for _ in range(self.num_of_experts)])
        self.seg_outputs = nn.ModuleList([nn.ModuleList(self.seg_outputs) for _ in range(self.num_of_experts)])
        if self.upscale_logits:
            self.upscale_logits_ops = nn.ModuleList(
                self.upscale_logits_ops) # lambda x:x is not a Module so we need
            # to distinguish here
